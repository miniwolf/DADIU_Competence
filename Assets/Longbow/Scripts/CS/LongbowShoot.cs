...
namespace Assets.Longbow.Scripts.CS {
	public class LongbowShoot : MonoBehaviour {
		...
		private const int MAX_POWER = 2000; //max force applied to the arrow
		...
		void Update () {
			...
			if(Input.GetMouseButton(0)) {
				...
				if(power < MAX_POWER) { //power will not exceed 2000
					power += MAX_POWER * Time.deltaTime; //add 2000 to power every second
				}
			}
